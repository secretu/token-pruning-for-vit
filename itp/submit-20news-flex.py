...
def single_job(arg):
    ...
    # do it quietly
    command = f"python token_pruning.py submit --target sing_research --task {task} --sparsity {s} --location {prune_location} --learning_rate {l} --reg_learning_rate {r} --sparsity_reg_loss_alpha {alpha} --warmup_epochs 10 --bin_num 512 --topk 60 --epochs 60 --eval_step 1000 --tag 0122scaledARP > /dev/null 2>&1"
    # print(command)
    # exit()
    os.system(command)
...
sparsity_reg_loss_alpha = [1e-3, 1e-4, 1e-6] # arp

...
def single_job(arg):
    ...
    # do it quietly
    command = f"python token_pruning.py submit --target sing_research --task {task} --sparsity {s} --location {prune_location} --learning_rate {l} --reg_learning_rate {r} --sparsity_reg_loss_alpha {alpha} --warmup_epochs {warmup_epoch} --bin_num 50 --epochs 40 --eval_step 2000 --topk 20 --tag 0129range"
    os.system(command)
...
lr = [4e-5]
reg_lr = [0.01]
sparsity_reg_loss_alpha = [0.001] # scaled arp 

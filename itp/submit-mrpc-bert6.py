...
def single_job(arg):
    ...
    # do it quietly
    command = f"python token_pruning_bert6.py submit --target sing_octo --task {task} --sparsity {s} --location {prune_location} --learning_rate {l} --reg_learning_rate {r} --sparsity_reg_loss_alpha {alpha} --warmup_epochs 80 --bin_num {bin_num} --topk {topk} --epochs 100 --eval_step 50 --tag 0129BERT6larger > /dev/null 2>&1"
    # print(command)
    # exit()
    os.system(command)

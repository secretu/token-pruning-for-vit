...
def single_job(arg):
    ...
    # do it quietly
    command = f"python token_pruning.py submit --target sing_octo --task {task} --sparsity {s} --location {prune_location} --learning_rate {l} --reg_learning_rate {r} --sparsity_reg_loss_alpha {alpha} --topk 50 --warmup_epochs {warmup_epoch} --epochs 4 --eval_step 5000 --bin_num 256 --tag 0201luck"
    # print(command)
    # exit()
    os.system(command)

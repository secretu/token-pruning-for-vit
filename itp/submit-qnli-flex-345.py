...
def single_job(arg):
    ...
    # do it quietly
    command = f"python token_pruning.py submit --target sing_research --task {task} --sparsity {s} --location {prune_location} --learning_rate {l} --reg_learning_rate {r} --sparsity_reg_loss_alpha {alpha} --warmup_epochs {warmup_epoch} --bin_num 50 --epochs 40 --eval_step 500 --tag 0130345scaledtopkARP2"
    os.system(command)

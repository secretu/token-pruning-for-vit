...
class CoFiTrainer(Trainer):
    ...
    def train(self):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))): #! 20 epoch
            ...
            for step, inputs in enumerate(train_dataloader):
                ...
                base_loss_step = loss_terms['base_loss']
                ...
                base_loss += base_loss_step if base_loss_step is not None else 0.0
                cls_kl_loss += cls_kl_loss_step if cls_kl_loss_step is not None else 0.0
                token_kl_loss += token_kl_loss_step if token_kl_loss_step is not None else 0.0
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                        ...
                        logs["token_kl_loss"] = (
                            token_kl_loss_scalar - logging_token_kl_loss_scalar) / self.args.logging_steps
    ...
    def training_step(self, model: torch.nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> List[torch.Tensor]:
        ...
        if self.teacher_model is not None:
            ...
            with torch.no_grad():
                teacher_outputs = self.teacher_model(pixel_values=inputs['pixel_values'],attention_mask=inputs['attention_mask'])
            cls_t, token_t = teacher_outputs['logits'],teacher_outputs['feature_logits']
            cls_s,token_s = model_output['logits'],model_output['feature_logits']

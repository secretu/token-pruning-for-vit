...
class L0ModuleForMAC(L0Module):
    ...
    def __init__(self,
                 ...
                 bin_num=50,
                 disable_layer_gate=False,
                 include_padding=False,
                 select_token_pruning_layer=None,
                 ):
        super().__init__(
            config=config,
        ...
        self.config = config
        self.mask_criterion = torch.nn.SmoothL1Loss()
        self.disable_layer_gate = disable_layer_gate
        self.include_padding = include_padding
        self.select_token_pruning_layer = select_token_pruning_layer
    ...
    def get_mac_and_constraint(
        ...
        if "token" in self.types:
            ...
            # full_macs = 0
            # relative_macs = 0
            # for full_length in full_lengths:
            #     full_macs = self.calculate_mac_for_model(
            #         token_length_for_each_layer=[full_length] * self.num_hidden_layers
            #     )
            #     # [CLS] token is never pruned
            #     macs = self.calculate_mac_for_model(
            #         token_length_for_each_layer=[t * (full_length - 1) + 1 for t in token_length_for_each_layer_list]
            #     )
            #     relative_macs += macs / full_macs
            # relative_macs /= len(full_lengths)
    ...
    def mask_regularization(self, pruned_steps):
        token_score = 1 - self.cdf_qz(0, self.token_loga)
        pruner_gate = torch.round(1 - self.cdf_qz(0, self.pruner_loga)) == 1.0
        effective_remain_token_lengths = torch.sum(token_score, dim=1).detach().round().clamp(0, token_score.shape[-1]).long()
        perfect_mask = torch.zeros_like(token_score, dtype=torch.float32, device=token_score.device)
        for i in range(token_score.shape[0]):
            perfect_mask[i, :effective_remain_token_lengths[i]] = 1.0
        perfect_mask = perfect_mask.detach()
        if pruner_gate.any():
            loss = self.get_mask_lambda(pruned_steps) * self.mask_criterion(
                token_score[pruner_gate],
                perfect_mask[pruner_gate],
            )
        else:
            loss = torch.tensor(0.0, device=token_score.device)
        return loss

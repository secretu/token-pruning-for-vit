...
class CoFiBertForSequenceClassification(BertForSequenceClassification):
    def __init__(self, config, token_prune_loc=None):
        ...
        self.bert = CoFiBertModel(config, token_prune_loc=token_prune_loc)
...
class CoFiBertModel(BertModel):
    def __init__(self, config, token_prune_loc=None):
        super().__init__(config)
        self.encoder = CoFiBertEncoder(config, token_prune_loc=token_prune_loc)
...
class CoFiBertEncoder(BertEncoder):
    ...
    def forward(
        ...
        prev_decision = torch.where(
            attention_mask[..., 1:].reshape((B, seq_len - 1, 1)) > -1,   # weisha dayu -1
